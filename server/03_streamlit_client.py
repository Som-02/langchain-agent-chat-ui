...
if st.sidebar.button("Download PDF"):
    if st.session_state.messages:
        ...
        filename = f"{timestamp}_chat_history.pdf"
        # downloads_dir = os.path.join(os.path.expanduser("~"), "Downloads")
        downloads_dir= os.path.dirname(os.path.abspath(__file__))
        downloads_dir = os.path.join(downloads_dir, "Downloads Pdf Reports")
        ...
        for msg in st.session_state.messages:
            ...
            chat_html += f"""
            <div style="margin: 10px 0; padding: 10px 14px; background: {bg_color};
                        border-left: 4px solid {border_color}; border-radius: 4px;">
                <div style="margin-bottom: 5px; font-size: 13px; color: #555;">{label}</div>
        ...
        <style>
            ...
            <h2 style="color: #333; border-bottom: 2px solid #ddd; padding-bottom: 8px;">
                Chat History — {datetime.now().strftime("%Y-%m-%d %H:%M")}
            </h2>
            {chat_html}
        </body></html>"""
